...
from PyQt5.QtGui import QFont, QIntValidator, QKeySequence, QPalette, QPixmap, QDesktopServices
...
from snekbooru_linux.ui.styling import PYGMENTS_CSS
from snekbooru_linux.api.booru import (fetch_custom_booru_posts,
                                 suggest_custom_booru_tags)
from snekbooru_linux.common.constants import DEFAULT_HOTKEYS, USER_AGENT
from snekbooru_linux.common.translations import SUPPORTED_LANGUAGES, _tr
from snekbooru_linux.core.config import (SETTINGS,
                                   save_custom_boorus,
                                   save_settings)
from snekbooru_linux.ui.styling import (EXAMPLE_STYLESHEET, get_fonts_path,
                                  get_themes_path, load_custom_themes,
                                  sCSSHighlighter, CodeEditor, 
                                  SCSS_KEYWORDS, SCSS_PROPERTIES, SCSS_PSEUDO)
from snekbooru_linux.ui.widgets import AdBlocker
from snekbooru_linux.core.workers import ApiWorker
from snekbooru_linux.core.workers import AsyncApiWorker
from snekbooru_linux.ui.vlc_player import VLCVideoPlayer
from snekbooru_linux.core.book_export import (export_epub_from_images,
                                        ...
                                        list_image_files)
from snekbooru_linux.core.temp_cache import snekbooru_temp_dir, snekbooru_temp_root
from snekbooru_linux.api.nhentai import download_gallery_pages
...
class BaseDialog(QDialog):
    """Base dialog with a custom title bar."""
    def __init__(self, title, parent=None):
        ...
        self.root_layout.setContentsMargins(1, 1, 1, 1) # Small margin for border
    ...
    def start(self):
        ...
        def work():
            ...
            if kind == "nhentai":
                ...
                if not url:
                    raise RuntimeError("Missing nhentai url")
    ...
    def open_folder(self):
        try:
            if self.images_folder and os.path.isdir(self.images_folder):
                QDesktopServices.openUrl(QUrl.fromLocalFile(self.images_folder))
        except Exception:
            pass
...
class HentaiSeriesDialog(BaseDialog):
    ...
    def on_episode_selected(self, item: QListWidgetItem):
        ...
        from snekbooru_linux.ui.main_window import _get_hhaven_stream_url # Import the new async helper
...
class HentaiViewerDialog(BaseDialog):
    ...
    def __init__(self, stream_url, title, parent=None):
        ...
        self.media_stack.addWidget(self.loading_label)

        # Video Widget (using VLC player for proper HLS support)
        ...
        self.vlc_video_player.error.connect(self.on_player_error)

        # Load media
        self.media_stack.setCurrentWidget(self.loading_label)
        ...
        try:
            ...
            if '.m3u8' in url:
                ...
                if streams:
                    ...
                    # VLC will handle the HLS playlist properly
                    return url, None
    ...
    def _on_stream_url_resolved(self, data, err):
        ...
        self.vlc_video_player.load(final_url)
    ...
    def seek_video(self, position_ms):
        ...
        self.vlc_video_player.seek(position_ms)
    ...
    def toggle_mute(self):
        """Toggle mute state."""
        self.vlc_video_player.toggle_mute()
        if self.vlc_video_player.is_muted():
            self.mute_btn.setIcon(qta.icon('fa5s.volume-mute'))
    ...
    def toggle_fullscreen(self):
        ...
        if self.vlc_video_player.is_fullscreen:
...
class SettingsDialog(BaseDialog):
    ...
    def import_theme(self):
        import shutil
        filepath, _ = QFileDialog.getOpenFileName(self, _tr("import snekbooru_linux Theme"), "", _tr("Snekbooru Stylesheets (*.snek.css);;All Files (*)"))
        if filepath:
            try:
    ...
    def _toggle_booru_enabled(self, item):
        ...
        from snekbooru_linux.core.config import save_settings
...
"""
    ...
    def values(self):
        ...
        if not dl_dir:
            dl_dir = os.path.join(QStandardPaths.writableLocation(QStandardPaths.AppLocalDataLocation), "Snekbooru", "data")
...
class FirstRunDialog(BaseDialog):
    def __init__(self, parent=None):
        ...
        default_dl_path = os.path.join(QStandardPaths.writableLocation(QStandardPaths.AppLocalDataLocation), "data")
