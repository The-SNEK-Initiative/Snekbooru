...
class HentaiEpisode(pydantic.BaseModel):
    ...
    def __init__(self, **data: typing.Any):
        ...
        if "hentai_content" in processed:
            processed["hentai_description"] = processed.pop("hentai_content")
        elif "hentai_description" not in processed:
            processed["hentai_description"] = ""
        ...
        if data.get("next_episode") is not None and isinstance(data["next_episode"], dict):
        ...
        if data.get("prev_episode") is not None and isinstance(data["prev_episode"], dict):
            for key, value in data.items():
                if key not in ("id", "name", "thumbnail", "date", "slug"):
                    if key == "next_episode" or key == "prev_episode":
...
class Hentai(pydantic.BaseModel):
    ...
    def __init__(self, **data: typing.Any):
        ...
        elif "description" not in processed:
