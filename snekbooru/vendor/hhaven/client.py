        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    self._logger.error("Failed to decode JSON after %d attempts: %s", max_retries, e)
                    raise
                self._logger.warning("JSON decode error on attempt %d/%d, retrying in 0.5s...", attempt + 1, max_retries)
